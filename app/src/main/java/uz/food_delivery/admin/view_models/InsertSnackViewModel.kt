...
class InsertSnackViewModel : ViewModel {
    ...
    companion object {
        ...
        lateinit var isLoading: MutableLiveData<Boolean>
         var insertDessertViewModel: InsertSnackViewModel? = null
        ...
        fun getInstance(context: Context, productType: ProductType): InsertSnackViewModel {
            if (insertDessertViewModel == null) {
                insertDessertViewModel = InsertSnackViewModel(context, productType)
    ...
    fun addSnack(): Boolean {
        snack.name = name
        snack.price = price
        snack.description = description
        snack.images = imageUrls.value as ArrayList<String?>
        var key = referance.push().key
        snack.key = key

        referance.child("snacks/$key").setValue(
            snack)
        Toast.makeText(context, snack.name + " is added!", Toast.LENGTH_SHORT).show()
        return false
    }
