...
class InsertDessertViewModel : ViewModel {
    ...
    companion object {
        ...
        lateinit var isLoading: MutableLiveData<Boolean>
         var insertDessertViewModel: InsertDessertViewModel? = null
        ...
        fun getInstance(context: Context, productType: ProductType): InsertDessertViewModel {
            if (insertDessertViewModel == null) {
                insertDessertViewModel = InsertDessertViewModel(context, productType)
    ...
    fun addDessert(): Boolean {
        dessert.name = name
        dessert.price = price
        dessert.description = description
        dessert.images = imageUrls.value as ArrayList<String?>
        var key = referance.push().key
        dessert.key = key

        referance.child("desserts/$key").setValue(dessert)
        Toast.makeText(context, dessert.name + " is added!", Toast.LENGTH_SHORT).show()
        return false
    }
